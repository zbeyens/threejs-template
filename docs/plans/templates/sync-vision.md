...
- source audit of root `VISION.md` and owner files for captured/reaffirmed
  rules
- JSON parse of `docs/sync/vision/status.json`
- `bun install` and mirror audit when `.agents/rules/**` changed
- final `check-complete.mjs`

Constraints:
- Latest-state doctrine only. No changelog prose in vision docs.
- Root `VISION.md` is the single project vision source.
...
Boundaries:
- Allowed sync inputs: `VISION.md`, `.agents/AGENTS.md`, `.agents/rules/**`,
  `docs/plans/**`, `docs/sync/**`, `docs/architecture/**`,
  `docs/brainstorms/**`, `docs/forms/**`, `docs/model/**`,
  `docs/operations/**`, `docs/gdds/**`, `docs/research/**`,
  `docs/sources/**`, and other changed Markdown-like root docs.
...
- Allowed edits: `VISION.md`, `docs/sync/vision/**`, this plan, and owner
  `.agents/rules/**` only when a workflow miss is proven.
...
- [ ] Root `VISION.md` is the only vision doctrine target. Evidence: no
      `docs/vision` directory remains unless explicitly reintroduced later.
...
| Vision doctrine update | pending | Patch or reaffirm root `VISION.md` | pending |
...
| Agent source / generated sync | pending | Run `bun install` when `.agents/rules/**` changed | pending |
