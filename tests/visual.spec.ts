...
async function sampleCanvas(page: Page): Promise<CanvasSample> {
  ...
  if (!buffer) return { ok: false, reason: 'canvas-detached' };
...
test('renders a nonblank interactive game canvas', async ({ page }, testInfo) => {
  ...
  if (testInfo.project.name.includes('mobile')) {
    ...
    if (box) {
      ...
      await expect
        .poll(async () => page.evaluate(() => window.__THREE_GAME_DIAGNOSTICS__?.player.speed ?? 0))
  ...
  await expect
    .poll(async () =>
      page.evaluate((start) => {
        const position = window.__THREE_GAME_DIAGNOSTICS__?.player.position ?? start;
