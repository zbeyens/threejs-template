...
6. Verify the complete procedural input trace in the fast test lane; capture a
   bounded screenshot/console spot check only when the visible surface changed.
...
5. Add diagnostics.
6. Verify with procedural player-input intents and one failing edge case.
...
- `npm test`.
- One bounded runtime capture when visual judgment matters.
- Main input path covered by deterministic simulation.
