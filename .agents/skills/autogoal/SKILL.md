...
`docs` for docs-dominant work, and repo-specific templates for domain lanes.
...
supervises multiple independently owned task plans, such as one GDD/full-loop
...
- heavyweight architecture or proposal work: use `--template task` with the
  relevant packs, or a repo-specific template if one exists
...
route, browser surface, or GitHub surface unless the plan explains why it is
the owning surface.
...
task-sized plans. This is the right shape for a GDD/full-loop parent plan that
delegates one child plan per GitHub issue or implementation packet.

Format the parent section as:

```md
Linked plans:
- [GitHub issue #123 task closeout](docs/plans/GH-123-auth-invites.md) - owns invite acceptance.
- [GitHub issue #124 task closeout](docs/plans/GH-124-password-reset.md) - owns reset flow.
```
