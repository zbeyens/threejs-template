---
description: Sync root VISION.md from changed Sira human and agent inputs. Use when the user invokes sync-vision, asks to evolve product/design/sensitivity/gameplay taste from recent plans/docs/skills, wants vision docs updated from accumulated human answers, or wants only changed inputs since the last recorded commit analyzed.
...
`VISION.md` is dynamic. Treat it like the project doctrine layer: it should
learn from repeated human answers, agent misses, plans, docs, source research,
and review handoffs, then forget one-off noise.

This skill is the incremental sync lane. It does not replace the `vision`
router. It keeps root `VISION.md` current by analyzing only changed durable
inputs since the last synced commit. Root `VISION.md` is the mandatory essential
read and single project vision source.
...
- `owner-routed`: belongs in a skill/rule, research doc, product/model doc,
  implementation plan, or proof target instead of vision docs.
...
taste, source order, cross-boundary law, product/model direction, proof
standards, owner-specific doctrine, or supervisor stop conditions.

Do not promote temporary command output, one-off route state, old branch
summaries, raw source dumps, generated mirror noise, artifact paths, or broad
history that does not change a future decision.
...
8. Patch root `VISION.md` only for `captured` clusters. Write current-state
   doctrine, not changelog prose.
9. Route owner-specific misses to the owner:
  - skill/rule miss -> `.agents/rules/**`, then `npx skiller@latest apply`;
   - post-merge/current-tree closure miss -> `autoclosure`;
   - code-shape/deslop miss -> `architecture-cleanup`;
   - hard removal miss -> `hard-cut`;
   - product/model doctrine -> `docs/model/**`, `docs/architecture/**`,
     `docs/gdds/**`, or `docs/brainstorms/**`;
   - source research doctrine -> `docs/sources/**`, `docs/research/**`, or
     `docs/forms/**`;
   - implementation doctrine -> the owning plan/template/rule.
10. Record a decision ledger in the plan or run summary.
11. If `.agents/rules/**` changed, run `npx skiller@latest apply` and mirror audit.
...
- a candidate changes product/API/model taste and no default can be inferred;
...
Include base commit, target commit, whether baseline advanced, changed input
count, candidate count, root vision doc changes, reaffirmed sections,
rejected/noise clusters, owner-routed follow-ups, deferred questions, changed
files, commands run, and next `sync-vision` command.
