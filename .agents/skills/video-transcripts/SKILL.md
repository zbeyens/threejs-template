...
description: Generate structured video transcripts from local files or GitHub video URLs using Gemini Files API. Use when a GitHub issue, PR, comment, or attachment includes a screen recording, .mov, .mp4, or hosted video and you need a <video-transcripts> block instead of hand-written notes.
...
For private GitHub asset URLs, the helper retries with `GITHUB_TOKEN`,
`GH_TOKEN`, or `gh auth token` when available.

## Use This When

- A GitHub issue, PR, or comment includes a screen recording.
...
3. For GitHub work, the canonical shared cache should live in GitHub next to
   the evidence it describes.
4. If the video is in the issue or PR body, use a top-level GitHub comment.
5. If the video is in a GitHub issue or PR comment, use one dedicated top-level cache comment for that source comment's video set. GitHub has no replies, so keep cache comments separated by source container instead of merging unrelated comment videos together.
...
12. For signed hosted URLs, strip the query string so a new signature does not
    invalidate an otherwise valid cache entry.
...
16. Do not use `docs/` for raw GitHub transcript cache. That is durable repo
    knowledge space, not raw issue evidence.
