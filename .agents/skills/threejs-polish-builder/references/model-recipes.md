...
Use these recipes only when `game-polish` records that BlenderMCP is unnecessary
for the current gap, or for repeated low-importance support geometry. Never use
procedural recipes as a fallback for a hero character or creature, primary
building or landmark, bespoke terrain or set piece, hero prop, or another asset
assigned to BlenderMCP.
...
For a game that asks for premium or showcase quality, build at least:
